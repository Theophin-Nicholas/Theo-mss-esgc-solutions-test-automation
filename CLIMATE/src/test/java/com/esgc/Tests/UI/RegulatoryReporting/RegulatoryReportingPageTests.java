...
public class RegulatoryReportingPageTests extends UITestBase {
    ...
    public void verifyPortfolioListForUserTest() {
        ...
        for(WebElement coverage: reportingPage.coverageList){
            assertTestCase.assertTrue(coverage.getText().matches("\\d+%"),"Coverage column is present with % of coverage as its value");
        }
...
//        assertTestCase.assertFalse(regulatoryReportingPage.getPortfolioList().contains(newPortfolioName),"New Portfolio is deleted from the list");
    ...
    public void verifySelectReportingAndPortfoliosColumnsTest() {
        ...
        reportingPage.selectPortfolioOptionByIndex(1);
        assertTestCase.assertTrue(reportingPage.interimReportsToggle.isDisplayed(),"Interim reports toggle is displayed");
        assertTestCase.assertTrue(reportingPage.annualReportsToggle.isDisplayed(),"Annual reports toggle is displayed");
        assertTestCase.assertTrue(reportingPage.useLatestDataToggle.isDisplayed(),"Use latest data toggle is displayed");
