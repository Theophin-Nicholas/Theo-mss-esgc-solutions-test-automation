...
public class RegulatoryReportingQueries {
    ...
    public List<Map<String, Object>> getSFDRCompanyOutput(String portfolioId, String year) {
        ...
        if(year.toLowerCase().equals("latest"))
            query = "with p as (SELECT * FROM VW_SFDR_REPORT_COMPANY_OUTPUT WHERE PORTFOLIO_ID='"+portfolioId+"' \n" +
                    "AND DATE_BASED_DATA_TYPE='yearly'),\n" +
                    "q as (select * from p where SFDR_1TXNMYID_1<>'NULL' or SFDR_1TXNMYID_2<>'NULL' or SFDR_1TXNMYID_3<>'NULL' qualify row_number() OVER (partition BY company_name ORDER BY company_name, YEAR DESC) =1)\n" +
                    "select * from p where COMPANY_NAME not in (select COMPANY_NAME from q) qualify row_number() OVER (partition BY company_name ORDER BY company_name, YEAR DESC) =1\n" +
                    "union all \n" +
                    "select * from q";
        else query = "SELECT * FROM VW_SFDR_REPORT_COMPANY_OUTPUT\n" +
    ...
    public List<Map<String, Object>> getBVD9idForEachPortfolio(String portfolioId){
        String query = "select * from \"DF_TARGET\".\"DF_PORTFOLIO\" where portfolio_id = '"+portfolioId+"'";
        System.out.println("query = " + query);
        return getQueryResultMap(query);
    }
