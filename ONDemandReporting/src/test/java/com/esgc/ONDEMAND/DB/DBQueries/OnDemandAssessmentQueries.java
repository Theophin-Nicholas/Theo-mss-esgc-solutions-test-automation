...
public class OnDemandAssessmentQueries {
    ...
    public List<Map<String,Object >> getEntitiesWithEsgDataOnly (){
        String query = "WITH dates\n" +
                "AS (\n" +
                "SELECT orbis_id BVD9_NUMBER\n" +
                ",MAX(SCORED_DATE) updated_date\n" +
                "FROM DF_TARGET.esg_overall_scores\n" +
                "// where orbis_id='503976450'\n" +
                ...
                "FROM (\n" +
                "SELECT BVD9_NUMBER\n" +
                ",MAX(PRODUCED_DATE) updated_date\n" +
                "FROM DF_TARGET.brown_share\n" +
                "GROUP BY BVD9_NUMBER\n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                ...
                "SELECT BVD9_NUMBER\n" +
                ",MAX(PRODUCED_DATE) updated_date\n" +
                "FROM DF_TARGET.CARBON_FOOTPRINT\n" +
                "GROUP BY BVD9_NUMBER\n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                "SELECT BVD9_NUMBER\n" +
                ",MAX(RESEARCH_DATE) updated_date\n" +
                "FROM DF_TARGET.temperature_alignment\n" +
                "GROUP BY BVD9_NUMBER\n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                "SELECT BVD9_NUMBER\n" +
                ",MAX(PRODUCED_DATE) updated_date\n" +
                "FROM DF_TARGET.physical_risk_management\n" +
                "GROUP BY BVD9_NUMBER\n" +
                "\n" +
                "UNION ALL\n" +
                "\n" +
                "SELECT ENTITY_ID_BVD9 BVD9_NUMBER\n" +
                ",MAX(RELEASE_DATE) updated_date\n" +
                "FROM DF_TARGET.entity_score\n" +
                "GROUP BY ENTITY_ID_BVD9\n" +
                ") A\n" +
                "// where BVD9_NUMBER='276126057'\n" +
                "GROUP BY BVD9_NUMBER\n" +
                ")\n" +
                ",ENT_MASTER\n" +
                "AS (\n" +
                ...
                ",ENTITY_PROPER_NAME\n" +
                ",CASE\n" +
                "WHEN managed_type = 'Subsidiary'\n" +
                "THEN PARENT_ORBIS_ID\n" +
                "END AS PARENT_ORBIS_ID\n" +
                ",CASE\n" +
                "WHEN managed_type = 'Subsidiary'\n" +
                "THEN PARENT_ENTITY_NAME\n" +
                "END AS PARENT_ENTITY_NAME\n" +
                ",MANAGED_TYPE\n" +
                "FROM DF_TARGET.ESG_ENTITY_MASTER\n" +
                "WHERE ENTITY_STATUS = 'Active'\n" +
                "AND managed_type <> 'Self-Assessed' qualify row_number() OVER (\n" +
                "PARTITION BY ORBIS_ID ORDER BY as_of_date DESC\n" +
                ",ENTITY_PROPER_NAME ASC\n" +
                ") = 1\n" +
                ")\n" +
                ",ENT_MASTER_FINAL\n" +
                "AS (\n" +
                "SELECT ENT_MASTER.ORBIS_ID\n" +
                ",ENT_MASTER.ENTITY_PROPER_NAME\n" +
                ",ENT_MASTER.PARENT_ORBIS_ID\n" +
                ",ENT_MASTER.PARENT_ENTITY_NAME\n" +
                ",ENT_MASTER.MANAGED_TYPE\n" +
                ",d.bvd9_number\n" +
                ",d.updated_date\n" +
                "FROM ENT_MASTER\n" +
                "JOIN dates d ON ENT_MASTER.ORBIS_ID = d.BVD9_NUMBER\n" +
                ")\n" +
                "SELECT ORBIS_ID\n" +
                ",ENTITY_PROPER_NAME\n" +
                "FROM ENT_MASTER_FINAL;";

        return DatabaseDriver.getQueryResultMap(query);
    }
    public List<Object> getEntityProperNameFromEntitiesWithEsgDataOnly(){
        ...
        rowList.addAll(DatabaseDriver.getColumnData(query, "ENTITY_PROPER_NAME"));
        return rowList;
    }
